#!/bin/bash
# P0: New text training with periodic checkpoint saving every 100 epochs
# Runs on a free GPU, does NOT kill existing training on GPU 3.
...
GPU="${GPU:-4}"
EXP_NAME="${EXP_NAME:-DNG-mp20-text-periodic-gpu4}"
EPOCHS="${EPOCHS:-3000}"
...
{
  echo "===== DNG Text Training (periodic ckpt) ====="
  echo "DATE=$(date '+%F %T')"
  echo "GPU=$GPU"
