...
def resolve_guide_factor(model, guide_factor):
    model_guide_threshold = getattr(model, 'guide_threshold', None)
    if model_guide_threshold is None:
        return guide_factor
    if guide_factor is None:
        return 1.0
    return guide_factor


def mean_pool_text(last_hidden_state, attention_mask):
...
def encode_texts(
    ...
    try:
        ...
        raise ImportError(
            "transformers is required for text-conditioned sampling. Install with `pip install -e .[text]`."
        ) from exc
    ...
    with torch.no_grad():
        for start in range(0, len(texts), batch_size):
            ...
            if pooling == 'cls':
                ...
                pooled = mean_pool_text(outputs.last_hidden_state, batch['attention_mask'])
            else:
...
def load_model(
    ...
    with initialize_config_dir(str(model_path), version_base="1.3"):
        ...
        try:
            ...
            model.scalers = torch.load(model_path / 'prop_scalers.pt', weights_only=False)
        ...
        if test_dataset_path is not None:
            for dataset_cfg in cfg.data.datamodule.datasets.test:
                dataset_cfg.path = str(test_dataset_path)
...
def structure_validity(crystal, cutoff=0.5):
    ...
    dist_mat = dist_mat + np.diag(np.ones(dist_mat.shape[0]) * min(crystal.lattice.abc))
