...
class AutoScaleFit(AutomaticFit):
    ...
    def observe(self, x, y):
        ...
        if AutomaticFit.activeVar == self:
            nSamples = y.shape[0]
            self.variance_in += torch.mean(torch.var(x, dim=0)).to(dtype=torch.float32) * nSamples
            self.variance_out += torch.mean(torch.var(y, dim=0)).to(dtype=torch.float32) * nSamples
