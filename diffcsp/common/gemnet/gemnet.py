...
from ..data_utils import (
    ...
    decompose_symmetric_matrix,
)
from ..ocp_graph_utils import radius_graph_pbc_v2 as radius_graph_pbc
from ..ocp_graph_utils import get_pbc_distances_v2 as get_pbc_distances
from ..lattice_score import edge_score_to_lattice_score_frac_symmetric
...
class RBFBasedLatticeUpdateBlockFrac(RBFBasedLatticeUpdateBlock):
    ...
    def forward(
        ...
        for i in range(self.num_out):
            ...
            outs.append(lattice_update)  # [B, 3, 3, *]
        outs = torch.stack(outs, dim=-1).sum(-1)  # [B, 3, 3]
        # [Batch_size, 3, 3]
        return outs
...
class GemNetT(torch.nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__()
        scale_file = str(Path(__file__).with_name("gemnet-dT.json").resolve())
        ...
        if self.pred_type:
            ...
            self.type_out = nn.Sequential(
                ...
                nn.Linear(emb_size_atom // 2, type_out_dim),
            )
    ...
    def forward(
        ...
        lattice_out = decompose_symmetric_matrix(output["stress"])
        coord_out = output["forces"]
        if self.pred_type:
            type_out = self.type_out(output["node_embeddings"])
