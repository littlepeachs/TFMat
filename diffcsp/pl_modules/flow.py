...
class CSPFlow(BaseModule):
    def __init__(self, *args, **kwargs):
        ...
        cond_keys = []
        if self.hparams.get("conditions", None) is not None:
            cond_keys = list(self.hparams.conditions.get("cond_keys", []) or [])
        text_encoder_cfg = self.hparams.get("text_encoder", None)
        if self.guide_threshold is not None:
            cond_dim = 0
            if cond_keys:
                self.cond_emb = MultiEmbedding(**self.hparams.conditions)
                cond_dim += self.cond_emb.n_out
            else:
                self.cond_emb = None

            if text_encoder_cfg is not None:
                self.text_encoder = hydra.utils.instantiate(text_encoder_cfg)
                cond_dim += self.text_encoder.n_out
            else:
                self.text_encoder = None

            if cond_dim <= 0:
                raise ValueError("guidance is enabled but no numeric or text condition encoder is configured.")
            cemb_dim = cond_dim
        else:
            self.cond_emb = None
            self.text_encoder = None
    ...
    def _build_condition_embedding(self, batch):
        cemb_list = []

        if self.cond_emb is not None:
            cemb_list.append(
                self.cond_emb(**{key: batch.get(key) for key in self.cond_emb.cond_keys})
            )
        ...
        if self.text_encoder is not None:
            ...
            if text_input_ids is None or text_attention_mask is None:
                raise KeyError("Text-enabled model expects text_input_ids and text_attention_mask in the batch.")
            cemb_list.append(
                self.text_encoder(
        ...
        if not cemb_list:
            raise RuntimeError("No condition embeddings are available for a guided model.")
        if len(cemb_list) == 1:
            return cemb_list[0]
        return torch.cat(cemb_list, dim=1)
    ...
    def forward(self, batch, guide_threshold=None):
        ...
        if guide_threshold is None:
            ...
            if self.cond_emb is None and self.text_encoder is None:
                raise Exception("Model is not initialized with guidance")
            cemb = self._build_condition_embedding(batch)
